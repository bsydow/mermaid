// Domain Storytelling diagram database
// Stores actors, workobjects, edges, and groups
...
export interface DomainStoryEdge {
  from: string;
  to: string;
  label: string;
  noOfSeq?: number;
...
/**
 * Database for Domain Storytelling diagrams.
 * Stores actors, workobjects, and edges, and provides methods to manage them.
 * Use this class to add, clear, and retrieve domain story elements.
 */
export class DomainStoryDb implements DomainstorytellingDB {
  ...
  edges: DomainStoryEdge[] = [];
  private actorsById = new Map<string, DomainStoryActor>();
  private workobjectsById = new Map<string, DomainStoryWorkobject>();
  ...
  groups: Map<string, DomainStoryGroup> = new Map<string, DomainStoryGroup>();
  ...
  /**
   * Clears all actors, workobjects, edges, icon definitions, and groups from the database.
   */
  clear() {
    this.actors = [];
    this.workobjects = [];
  ...
  addGroup(id: string, title?: string, parentId?: string) {
    ...
    if (existing) {
      if (title && existing.title && existing.title !== title) {
        log.warn(
          `[DomainStory] Conflicting group title for '${id}'. Keeping '${existing.title}' and ignoring '${title}'.`
        );
      }
      this.groups.set(id, {
        id,
  ...
  setActorGroup(actorId: string, groupId: string) {
    ...
    if (actor) {
      ...
      // Actor may not be added yet; store pending assignment for later resolution
      // We'll resolve in the renderer by looking up group memberships from db
  ...
  setWorkobjectGroup(workobjectId: string, groupId: string) {
    ...
    if (workobject) {
      ...
      // Workobject may not be added yet; store pending assignment for later resolution.
      this.pendingGroupMemberships.set(workobjectId, groupId);
    }
  }

  getGroups(): DomainStoryGroup[] {
  ...
  addActor(id: string, label?: string) {
    ...
    if (existingActor) {
      ...
      if (!existingActor.label || existingActor.label === fallbackLabel) {
        ...
        log.warn(
          `[DomainStory] Conflicting actor label for '${id}'. Keeping '${existingActor.label}' and ignoring '${label}'.`
        );
      }
      return;
    }
    ...
    const actor: DomainStoryActor = { id, label: label ?? fallbackLabel, icon, group };
  ...
  addWorkobject(id: string, label?: string, iconTarget?: string) {
    ...
    if (existingWorkobject) {
      if (!existingWorkobject.label || existingWorkobject.label === fallbackLabel) {
        ...
        log.warn(
          `[DomainStory] Conflicting workobject label for '${id}'. Keeping '${existingWorkobject.label}' and ignoring '${label}'.`
        );
      }
      return;
    }
    ...
    const group = this.pendingGroupMemberships.get(id);
    this.pendingGroupMemberships.delete(id);
    const workobject: DomainStoryWorkobject = { id, label: resolvedLabel, icon, group };
    this.workobjects.push(workobject);
    this.workobjectsById.set(id, workobject);
  }

  /**
   * Drops `group` assignments on actors and workobjects that reference a
   * group id that was never declared. Mutates the matching entries in
   * place (sets `group` to undefined) and logs a warning per dropped ref.
   * Intended to run once after all declarations and sentences are processed.
   */
  pruneUnknownGroupReferences() {
    this.actors.forEach((actor) => {
      if (actor.group && !this.groups.has(actor.group)) {
        log.warn(
          `[DomainStory] Unknown group '${actor.group}' for actor '${actor.id}'. Group assignment ignored.`
        );
    ...
    this.workobjects.forEach((workobject) => {
      if (workobject.group && !this.groups.has(workobject.group)) {
        log.warn(
          `[DomainStory] Unknown group '${workobject.group}' for workobject '${workobject.id}'. Group assignment ignored.`
        );
