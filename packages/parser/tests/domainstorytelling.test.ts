...
import { DomainStory } from '../src/language/index.js';
...
describe('domainstorytelling', () => {
  ...
  it.each([
    ...
    const result = parse(context);
    expectNoErrorsOrAlternatives(result);
    expect(result.value.$type).toBe(DomainStory.$type);
  });
...
A_Customer : 01 -- "places" -> W_Order`;
    ...
    expect(astAny.workObjectDeclarations?.[0]?.icon).toBe('fa:fa-shopping-cart');
  });
