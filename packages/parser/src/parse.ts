...
const initializers = {
  ...
  domainstorytelling: async () => {
    const { createDomainStoryServices } = await import('./language/domainstorytelling/index.js');
    const parser = createDomainStoryServices().DomainStory.parser.LangiumParser;
...
export async function parse(diagramType: 'domainstorytelling', text: string): Promise<DomainStory>;
